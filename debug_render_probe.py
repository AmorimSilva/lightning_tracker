...
fetch_start_utc = max(_to_utc(plot_end) - timedelta(seconds=max(20, settings.aws_interval_seconds)), _to_utc(plot_start))
...
if dl.downloaded:
    ...
    def worker() -> None:
        nonlocal bg, error
        try:
            bg = provider.get_background(dt_utc=end_utc, extent=(-50, -40, -10, 0), diag={})
    ...
    print('background joined', round(time.time() - bg_step, 2), 'alive', thread.is_alive(), 'bg', bg is not None, 'error', error, flush=True)
